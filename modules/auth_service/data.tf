locals {
  ...
  rds_output_prefix  = "/${var.namespace}/${var.env}/main-aurora-01"
}
...
data "aws_ssm_parameter" "db_instance_address" {
  name = "${local.rds_output_prefix}/cluster_endpoint"
}
data "aws_ssm_parameter" "db_instance_port" {
  name = "${local.rds_output_prefix}/cluster_port"
}
data "aws_ssm_parameter" "db_database_name" {
  name = "${local.rds_output_prefix}/cluster_database_name"
}
