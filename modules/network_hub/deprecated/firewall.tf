...
locals {
  ...
  # All internal CIDRs across all VPCs
  super_cidr_block = "10.0.0.0/8" # TODO abstract this to a var
  # [aws_vpc.spoke_vpc_a.cidr_block, aws_vpc.spoke_vpc_b.cidr_block]
}
  ...
  rule_group {
    ...
    rules_source {
      rules_source_list {
        ...
        targets              = [".google.com", "ai.ikenley.com"]
      }
    }
...
resource "aws_cloudwatch_log_group" "anfw_alert_log_group" {
  name = "/aws/${local.id}-firewall/alert"
}

resource "aws_cloudwatch_log_group" "anfw_alert_log_group_flow" {
  name = "/aws/${local.id}-firewall/flow"
}

resource "aws_cloudwatch_log_group" "anfw_alert_log_group_tls" {
  name = "/aws/${local.id}-firewall/tls"
}
...
#   bucket = module.anfw_flow_bucket.s3_bucket_id
#   rule {
...
resource "aws_networkfirewall_logging_configuration" "anfw_alert_logging_configuration" {
  ...
  firewall_arn = aws_networkfirewall_firewall.inspection_vpc_anfw[0].arn
  logging_configuration {
    ...
    log_destination_config {
      log_destination = {
        logGroup = aws_cloudwatch_log_group.anfw_alert_log_group_flow.name
      }
      log_destination_type = "CloudWatchLogs"
      log_type             = "FLOW"
    }
    log_destination_config {
      log_destination = {
        logGroup = aws_cloudwatch_log_group.anfw_alert_log_group_tls.name
      }
      log_destination_type = "CloudWatchLogs"
      log_type             = "TLS"
    }
